...
class JensNewnsAnderson:
    ...
    def __post_init__(self):
        ...
        self.eps = np.linspace(-25, 20, 50000)
        self.delta0 = 2
    ...
    def fit_parameters(self, eps_ds, alpha, beta, constant):
        ...
        # Constant offset for the energies 
        hybridisation_energy += constant
...
if __name__ == '__main__':
    ...
    REMOVE_LIST = ['Sc', 'Y', 'Co', 'Hf', 'Ta', 'Os'] #['Sc', 'Y', 'Hf', 'Nb', 'Ti', 'V']
    # REMOVE_LIST = ['Ti', 'V', 'Cr', 'Re', 'Ru', 'Os']
    KEEP_LIST = [] #['Ag', 'Au', 'Cu', 'Pt', 'Pd', 'Ru', 'Ir', 'Rh', 'Ni']
    ...
    # are positive
    constraints = [ [0, 0, -np.inf], [np.inf, np.inf, np.inf] ]
    popt, pcov = curve_fit(fitting_function.fit_parameters, parameters['d_band_centre'], dft_energies, bounds=constraints)  
    ...
    for i, metal in enumerate(metals):
        ...
        texts.append(ax.text(dft_energies[i], optimised_hyb[i], metal, color=colour, fontsize=12))
