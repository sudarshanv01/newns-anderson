...
if __name__ == '__main__':
    ...
    # Create the figure
    fige, axe = plt.subplots(2, 1, figsize=(7, 11), constrained_layout=True)
    ...
    for i, beta_p in enumerate(BETA_PRIME):
        ...
        for j, _epsilon_a in enumerate(EPSILON_A[i]):
            ...
            if analytical.has_localised_occupied_state_positive:
                axe[i].plot(analytical.root_positive, 0, '*', color=colors[j], ms=16, label='Bonding')
            ...
            # if analytical.has_localised_occupied_state_negative:
                axe[i].plot(analytical.root_negative, 0, '*', color=colors[j], ms=16, alpha=0.5, label='Anti-bonding')
