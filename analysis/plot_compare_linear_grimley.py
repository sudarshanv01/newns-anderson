"""Compare quantities from the current model with NAG."""
...
if __name__ == '__main__':
    """Compare the chemisorption, hybridisation
    and orthogonalisation energy variation with 
    the d-band centre for the Newns-Anderson-Grimley
    model and the linear repulsion model used in the
    main text of the manuscript. Make a separate figure
    for each adsorbate, each graph has only one row of
    continously varied transition metals."""
    ...
    REPULSION_TYPES = ['grimley', 'linear']
    REPULSION_LABELS = ['grimley', 'two-state']
    ...
    for i, (adsorbate, eps_a) in enumerate(zip(ADSORBATES, EPS_A_VALUES)):
        ...
        fig.savefig(f'output/compare_linear_grimley_adsorbate_{adsorbate}.png', dpi=300)