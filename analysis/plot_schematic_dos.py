...
def plot_pdos(energies, pdos, label, color):
    ...
    ax.plot(pdos, energies, '-', label=label, color=color)
    ax.fill_between(pdos, energies, color=color, alpha=0.2)
...
if __name__ == '__main__':
    ...
    for i, setup in enumerate(COMP_SETUP):
        ...
        energies, pdos, _ = pdos_data['slab']['Pt']
    ...
    ax.set_ylim([-10, 10])
    ax.set_xticks([])
    ax.set_xlabel('Projected Density of States')
    ax.set_ylabel('$\epsilon - \epsilon_F$ (eV)')


    fig.savefig('output/schematic_dos.png', dpi=300)